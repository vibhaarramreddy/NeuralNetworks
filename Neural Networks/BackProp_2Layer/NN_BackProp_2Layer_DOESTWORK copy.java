...
public class MultilayerPerceptron 
{
   ...
   public MultilayerPerceptron(int numInputs, int numOutputs, int[] numHidden)
   {
      ...
      Θj = new double[activations[1].length];
      Θi = new double[this.numOutputs];
   }
   ...
   public double[] calculateOutputs() 
   {
      ...
      for (int layers = 1; layers < activations.length; layers++)                  
      {
         ...
         { 
            double netInput = calculateActivationValue(layers, rows);
            ...
            activations[layers][rows] = netInput;         //Uses the propagation function to define values for all non-input nodes
         }
      }
         ...
         for (int testCases = 0; testCases < trainingValues.length; testCases++)
         {
            ...
            for (int leftA = 0; leftA < numInputs; leftA++)
            {
               for (int rightA = 0; rightA < activations[1].length; rightA++)
               {
                  ...
                  double hj = activationFunction(Θj[rightA]);
                  ...
                  for (int i = 0; i < numOutputs; i++)
                  {
                     ...
                     double Fi = activationFunction(Θi[i]);
                     double ωi = (trainingValues[testCases][i] - output);
                     
                     double ψi = ωi * derivative(Fi); 
                     
                     Ωj += ψi * weights[1][rightA][i];
                     
                     double deltaLeftWeight = (lambda) * hj * ψi;
                     
                     weights[1][rightA][i] += deltaLeftWeight;                            //second-layer weights
                  }
                  

                  double Ψj = Ωj * derivative(Θj[rightA]);
                  
                  double deltaRightWeight = (lambda) * activations[0][leftA] * Ψj;
                  
                  weights[0][leftA][rightA] += deltaRightWeight;                         //first-layer weights

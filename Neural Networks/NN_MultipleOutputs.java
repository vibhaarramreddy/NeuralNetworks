...
public class MultilayerPerceptron 
{
         ...
         double[][] trainingValues, double[][] inputValues, double initialError, double learningFactor, double growthFactor)
   {
      ...
      while (done == 0) 
      {
         ...
         for (int testCases = 0; testCases < trainingValues.length; testCases++)
         {
            ...
            //Calculates the delta weights values for second-layer weights 
            for (int leftA = 0; leftA < activations[1].length; leftA++) 
            {
               for (int i = 0; i < numOutputs; i++)
               {
                  double output = activations[2][i]; 
                  double secondLayerSum = 0.0;
                  
                  double deltaWeight = (-(trainingValues[testCases][i] - output)) * activations[1][leftA];
                  
                  for (int j = 0; j < activations[1].length; j++)
                  {
                     secondLayerSum += activations[1][j] * weights[1][j][i];
                  }
                  
                  deltaWeight *= (-lambda) * derivative(secondLayerSum);
                  
                  weights[1][leftA][i] += deltaWeight;
               }
            }
            
          //Calculates the delta weights values for first-layer weights 
            for (int leftA = 0; leftA < numInputs; leftA++)
            {
               for (int rightA = 0; rightA < activations[1].length; rightA++)
               {
                  double firstLayerSum = 0.0;
                  
                  for (int k = 0; k < activations[0].length; k++)
                  {
                     firstLayerSum += activations[0][k] * weights[0][k][rightA];
                  }
                  
                  double deltaWeight = (-activations[0][leftA]) * derivative(firstLayerSum);
                  ...
                  for (int i = 0; i < numOutputs; i++)
                  {
                     ...
                     outputWeight += ((trainingValues[testCases][i] - output) * derivative(secondLayerSum) * weights[1][rightA][i]);
                  }
                  
                  deltaWeight *= (-lambda) * outputWeight;
   ...
   public static void main(String[] args) throws FileNotFoundException
   {
      ...
      double initialError = tempPerceptron.calculateTotalError(initialIndividualErrors);     //inital error of the network using the randomized weights
      ...
      //Prints out diagonistic information about the trained network
      for (int testCases = 0; testCases < trainingValues.length; testCases++)
      {
         for (int i = 0; i < tempPerceptron.numOutputs; i++)
         {

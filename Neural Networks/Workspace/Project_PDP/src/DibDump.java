 ...
 */
   ...
   /**
    * Writes the activation values to a file
    * @param imageScaledArray
    * @throws Exception
    */
   public static void writeActFile(double[][] imageScaledArray) throws Exception 
   {
      try
      {
          PrintWriter out = new PrintWriter(new FileWriter("./src/InputActValues.txt"), true);
          for(int r = 0; r < imageScaledArray.length; r++)
          {
             for(int c = 0; c < imageScaledArray[c].length-1; c++)
             {
                out.println(imageScaledArray[r][c]);
             }
          }
          out.close();
      }
   ...
   public static double[] readActFile() throws FileNotFoundException
   {
      double[] inputValues = new double[144];
      Scanner sc1 = new Scanner(new FileReader("./src/InputActValues.txt"));
...
// The color table
      ...
      if (args.length > 0)
         ...
         inFileName = "./src/test1.bmp";

      if (args.length > 1)
         outFileName = args[1];
      else
         outFileName = "./src/test2.bmp";
 ...
 */
      ...
      double[][] imageScaledArray = new double[12][12];
      for(int r = 0; r < 12; r++)
      {
         for(int c = 0; c < 12; c++)
         {
            imageScaledArray[r][c] = ((imageArray[r][c])/1.7e7)+1;
         }
      }
      ...
      for(int r = 0; r < imageScaledArray.length; r++)
      {
         for(int c = 0; c < imageScaledArray[c].length-1; c++)
         {
            System.out.print(imageScaledArray[r][c] + " ");
         }
      }
      
      try {
         writeActFile(imageScaledArray);
      } catch (Exception e1) {
         // TODO Auto-generated catch block
         e1.printStackTrace();
      }
      
      double[] actValues = new double[144]; 
      try {
         actValues = readActFile();
      } catch (FileNotFoundException e1) {
         // TODO Auto-generated catch block
         e1.printStackTrace();
      }
      
      double[][] inputValues = new double[1][144];
      double[][] trainingValues = new double[1][144];
      
      for(int row = 0; row < actValues.length; row++)
      {
            inputValues[0][row] = actValues[row];
      }
      
      for(int row = 0; row < actValues.length; row++)
      {
            trainingValues[0][row] = inputValues[0][row];
      }
      
      int[] hiddenLayer = {4};
      MultilayerPerceptron tempPerceptron = new MultilayerPerceptron(144, 144, hiddenLayer);
      tempPerceptron.randomizeWeightValues(-0.01, 0.01);
      
      tempPerceptron.steepestDescentTraining(0.01, 10000, 0.0, inputValues, trainingValues, 3000.0, 5.0, 1.0);
      
      /**
      try {
         tempPerceptron.returnOutputValues();
      } catch (Exception e2) {
         // TODO Auto-generated catch block
         e2.printStackTrace();
      }
      */
      ...
      Scanner sc2;
      try {
         sc2 = new Scanner(new FileReader("./src/OutputValues.txt"));
         for(int r = 0; r < 12; r++)
         {
            for(int c = 0; c < 12; c++)
            {
               ...
               {
                  imageScaledArray[r][c] = sc2.nextDouble();
               }
            }
         }
      } catch (FileNotFoundException e1) {
      ...
      {
         for(int c = 0; c < imageArray[c].length-1; c++)
         {
            imageArray[r][c] = (int) ((imageScaledArray[r][c])*16777215);
         }
      }

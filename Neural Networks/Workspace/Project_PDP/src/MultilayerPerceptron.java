 ...
 * December 6th 2019
 * This class creates a simple multilayer perceptron that has configurable activation layers. 
 * The number of activations in each layer is user configurable.
 * 
 * Method Table of Contents:
 * MultilayerPerceptron(int numInputs, int numOutputs, int[] numHidden)
 * setInputValues(double[] inputValues)
 * randomNumberGenerator(double min, double max)
 * randomizeWeightValues(double min, double max)
 * setWeights()
 * propagationFunction(int actLayer, int actRow)
 * activationFunction(double netInput)
 * derivative(double netInput)
 * calculateActivationValue(int actLayer, int actRow)
 * calculateOutputs() 
 * calculateIndividualError(double calculatedAns, double trainingVal)
 * calculateTotalError(double[] calculatedErrors)
 * rollBackWeights(double[][][] oldWeights)
 * saveWeights(double[][][] oldWeights)
 * steepestDescentTraining(double errorThreshold, int iterationThreshold, double changeErrorThreshold, 
...
public class MultilayerPerceptron 
{
   ...
   public MultilayerPerceptron(int numInputs, int numOutputs, int[] numHidden)
   {
      ...
      Ωj = new double[numRows];
      Ωk = new double[numRows];
   }
   ...
   public double[] calculateOutputs() 
   {
      ...
      for (int layers = 1; layers < activations.length; layers++)                  
      {
         ...
         { 
            ...
            Θvalues[layers][rows] = netInput;                                 //Calculates theta values for the back propagation algorithm  
         ...
         double[][] trainingValues, double[][] inputValues, double initialError, double learningFactor, double growthFactor)
   {
      ...
      while (done == 0) 
      {
         ...
         for (int testCases = 0; testCases < trainingValues.length; testCases++)
         {
            ...
            for (int j = 0; j < activations[2].length; j++)
            {
               Ωj[j] = 0.0;
               
               for (int i = 0; i < numOutputs; i++)
               {
                  double Θi = Θvalues[3][i];
                  
                  double ωi = (trainingValues[testCases][i] - activations[3][i]);
                  
                  double ψi = ωi * derivative(Θi); 
                  
                  Ωj[j] += ψi * weights[2][j][i];
                  
                  weights[2][j][i] += (lambda) * activations[2][j] * ψi;       //third-layer weights
               }
            }
            ...
            //Calculates the delta weight values for the first hidden-layer for this test case
            ...
            for (int i = 0; i < numOutputs; i++)
            {
               individualErrors[i] = calculateIndividualError(activations[3][i], trainingValues[testCases][i]);
            }
   ...
   public static void main(String[] args) throws FileNotFoundException
   {
      ...
      for (int testCases = 0; testCases < trainingValues.length; testCases++)
      {
         for (int i = 0; i < tempPerceptron.numOutputs; i++)
         {
            ...
            errors[testCases][i] = tempPerceptron.calculateIndividualError(tempPerceptron.activations[3][i], trainingValues[testCases][i]);
         }
      }
      ...
      for (int testCases = 0; testCases < trainingValues.length; testCases++)
      {
         for (int i = 0; i < tempPerceptron.numOutputs; i++)
         {
            ...
            System.out.println("output: " + tempPerceptron.activations[3][i]);
            System.out.println("error: " + tempPerceptron.calculateIndividualError(tempPerceptron.activations[3][i], trainingValues[testCases][i]));
         }
      }
